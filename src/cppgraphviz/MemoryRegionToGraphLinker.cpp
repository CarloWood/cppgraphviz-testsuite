...
std::shared_ptr<GraphTracker> const& MemoryRegionToGraph::get_graph_tracker(
    MemoryRegion const& memory_region_key, std::shared_ptr<GraphTracker> const& default_graph) const
{
  DoutEntering(dc::notice, "MemoryRegionToGraph::get_graph_tracker(" << memory_region_key << ", " << default_graph << ")");
...
#ifdef CWDEBUG
void MemoryRegionToGraph::print_on(std::ostream& os) const
{
...
{
  DoutEntering(dc::notice, "MemoryRegionToGraphLinker::get_graph_tracker(" << default_graph << ", " << item_area << ")");
...
{
  DoutEntering(dc::notice, "MemoryRegionToGraphLinker::get_graph_tracker(" << item_area << ")");
...
void MemoryRegionToGraphLinker::start_new_subgraph_for(MemoryRegion memory_region, std::shared_ptr<GraphTracker> const& subgraph)
{
  DoutEntering(dc::notice, "start_new_subgraph_for(" << memory_region << ", " << subgraph << ")");
  Dout(dc::notice, "before:\n" << *this);

  // MemoryRegionToGraph, MemoryRegionToGraphLinker
  MemoryRegionToGraph memory_region_to_graph(memory_region, subgraph);
...
bool MemoryRegionToGraphLinker::erase_memory_region_to_graph(MemoryRegion const& item_area)
{
...
void MemoryRegionToGraphLinker::end_subgraph(MemoryRegion item_area)
{
...
#ifdef CWDEBUG
void MemoryRegionToGraphLinker::print_on_with_indentation(std::ostream& os, std::string indentation) const
{
  for (auto& key_value_pair : memory_region_to_graph_map_)
  {
    MemoryRegionToGraph const& memory_region_to_graph = key_value_pair.first;
    MemoryRegionToGraphLinker const& memory_region_to_graph_linker = key_value_pair.second;
